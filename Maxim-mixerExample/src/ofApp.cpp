...
void ofApp::audioOut(float * output, int bufferSize, int nChannels) {
    for (int i = 0; i < bufferSize; i++){
        ...
        output[i*nChannels    ] = mySine.sinewave(myFreq)*myCrossfade
        +mySample.play()*(1-myCrossfade); /* You may end up with lots of outputs. add them here */
        output[i*nChannels + 1] = mySine.sinewave(myFreq)*myCrossfade
        +mySample.play()*(1-myCrossfade);
    }
}
...
void ofApp::mouseMoved(int x, int y){
    ...
    myCrossfade=float(y)/ofGetHeight();
    cout << myCrossfade << endl;
}
